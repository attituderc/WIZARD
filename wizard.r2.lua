...
local function fieldIncDec(event, value, max)
  if event == EVT_PLUS_BREAK or event == EVT_ROT_LEFT or event == 
--@StodgySticks, chanded this from + to - to change menu direction
EVT_PLUS_REPT then
    value = (value - max)
    dirty = true
  elseif event == EVT_MINUS_BREAK or event == EVT_ROT_RIGHT or event == 
--@StodgySticks, chanded this from + to - to change menu direction
EVT_MINUS_REPT then
    value = (value - max + 2)
    dirty = true
